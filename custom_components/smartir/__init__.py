...
MANIFEST_URL = (
    ...
    "ntaapp/HomicIR/{}/"
    "custom_components/homicir/manifest.json")
REMOTE_BASE_URL = (
    "https://raw.githubusercontent.com/"
    "ntaapp/HomicIR/{}/"
    "custom_components/homicir/")
            ...
            async with session.get(MANIFEST_URL.format(branch)) as response:
                if response.status == 200:
                    ...
                    if StrictVersion(current_ha_version) < StrictVersion(min_ha_version):
                        hass.components.persistent_notification.async_create(
                            "There is a new version of HomicIR integration, but it is **incompatible** "
                            "with your system. Please first update Home Assistant.", title='HomicIR')
                        return

                    if do_update is False:
                        hass.components.persistent_notification.async_create(
                            "A new version of HomicIR integration is available ({}). "
                            "Call the ``homicir.update_component`` service to update "
                            "the integration. \n\n **Release notes:** \n{}"
                            .format(last_version, release_notes), title='HomicIR')
                        return
                    ...
                    if has_errors:
                        hass.components.persistent_notification.async_create(
                            "There was an error updating one or more files of HomicIR. "
                            "Please check the logs for more information.", title='HomicIR')
                    else:
                        hass.components.persistent_notification.async_create(
                            "Successfully updated to {}. Please restart Home Assistant."
                            .format(last_version), title='HomicIR')
    except Exception:
